...
- `added` — Required. Date the entry was added (`YYYY-MM-DD`).
- `tags` — Required on entries in content folders (`wiki/[category]/`, `log/`); hub pages in `wiki/hubs/` are exempt. Lowercase kebab-case, plural. Full rules in `CONVENTIONS.md` → Entry format.
- `context` — Optional. Structured taxonomy for which "world" the entry belongs to. Define your own small fixed set (e.g., `work`, `personal`, `ai`).
- `project` — Optional. Wikilink(s) to project page(s) under `wiki/projects/`.
