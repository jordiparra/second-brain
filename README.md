...
second-brain/
├── .claude-plugin/
│   └── marketplace.json    ← Marketplace manifest (lists the notes plugin)
├── plugins/                ← The notes plugin (installable via `claude plugin install`)
│   ├── .claude-plugin/
│   │   └── plugin.json     ← Plugin manifest (name: notes)
│   └── skills/
...
└── vault/                  ← The Obsidian vault (open this in Obsidian)
    ...
    ├── lists/              ← Base views for browsing (Books, Movies, Trips, …)
    ├── templates/          ← Entry templates (one per category)
...
The repo serves two roles: it's a **starter template** for the wiki (vault/) AND a **Claude Code plugin** (plugins/). Both live together so the schema and the tooling that operates on it evolve in lockstep.
...
### 2. Install the notes plugin

The repo is its own Claude Code marketplace. Add it once, then install the plugin:

```sh
claude plugin marketplace add ~/second-brain
claude plugin install notes@second-brain
```

Verify:

```sh
claude plugin list | grep notes
```

The four skills (`notes-ingest`, `notes-query`, `notes-lint`, `wrap`) become available in any Claude Code session.

### 3. Tell your global `~/.claude/CLAUDE.md` about the wiki

**Required.** The skills look up the wiki path in your global `~/.claude/CLAUDE.md` — without this section, they don't know where your notes live. Add a section like this:
...
Adjust the path if you cloned somewhere else (e.g. `~/Library/Mobile Documents/com~apple~CloudDocs/Documents/Notes/` if you keep your wiki in iCloud Drive).
